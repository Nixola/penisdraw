...
function love.draw()
  ...
  if not text then
    ...
    if #complexLine > 3 then
      love.graphics.push()
      love.graphics.translate(love.mouse.getPosition())
      love.graphics.setLineWidth(current_width)
      love.graphics.line(complexLine)
      love.graphics.pop()
    end
...
--[[
  ...
  function love.keypressed(key, is_repeat, blah)
    ...
    elseif command then
      ...
      if key == "return" and not is_repeat then
        ...
        if nixCommands[c] then
          local worked, err = pcall(nixCommands[c], unpack(t))
  ...
  function love.textinput(t)
    if not text and not command and t == ":" then 
      command = ":" 
      return
    end
...
function poly(n, radius, star)
  ...
  if star == "false" then star = false end
  local t = complexLine
...
function sendPoly(x, y)
  if not x or not y then
    x,y = love.mouse.getPosition()
  end
  x = tonumber(x)
  y = tonumber(y)
  for i = #complexLine/2, 1, -1 do
    if i == 1 then break end
    local y2, x2, y1, x1 = complexLine[i*2]+y, complexLine[i*2-1]+x, complexLine[i*2-2]+y, complexLine[i*2-3]+x
    send_data(serialize_line({x1, y1, x2, y2}))
    print(x2, y2)
  end
end
